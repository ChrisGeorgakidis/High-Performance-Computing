...

{
    ...
    #pragma omp parallel
    {
        ...
        /*The address of each cluster differs from the previous one by numCoords. That means that each cluster differs from the newClusters[0] by i*numCoords. This change makes the following for loop free of data depedencies*/
        #pragma omp for nowait schedule(static)
        for (i=1; i<numClusters; i++){
