...
__constant__ dataType d_Filter[33];
...
#define cudaErrorCheck() {\
  ...
  if (error != cudaSuccess) {\
    ...
    freeMemory(h_Input, h_Buffer, h_OutputCPU, h_OutputGPU, d_Input, d_Buffer, d_OutputGPU);\
    return (ERROR);\
  }\
}\
...
__global__ void convolutionRowSharedMem(dataType *Input, dataType *Output, int filterR, int imageW)
{
  ...
  for (k = -filterR; k <= filterR; k++){
    ...
    {
      //printf("ix=%d,iy=%d, k=%d, output_idx=%d, input_idx=%d\n", ix, iy, k, iy * imageWithPaddingW + ix, iy * imageWithPaddingW + d);
      sum += s_Input[ty * imageWithPaddingW + d] * d_Filter[filterR - k];
    }
    else
    {
...
__global__ void convolutionColumnSharedMem(dataType *Input, dataType *Output, int filterR, int imageW, int imageH)
{
  ...
  for (k = -filterR; k <= filterR; k++){
    ...
    {
      //printf("ix=%d,iy=%d, k=%d, output_idx=%d, input_idx=%d\n", ix, iy, k, iy * imageWithPaddingW + ix, iy * imageWithPaddingW + d);
      sum += s_Input[d * imageWithPaddingW + tx] * d_Filter[filterR - k];
    }
    else
    {
      sum += Input[iy * imageWithPaddingW + ix] * d_Filter[filterR - k];
  ...
  for (y = filterR; y < (imageH + filterR); y++) {
    for (x = filterR; x < (imageW + filterR); x++) {
      ...
      for (k = -filterR; k <= filterR; k++) {
        ...
        h_Dst[y * imageWithPaddingW + x] = sum;
      }
    }
  }
}
...
////////////////////////////////////////////////////////////////////////////////
int freeMemory(dataType *h_Input, dataType *h_Buffer, dataType *h_OutputCPU, dataType *h_OutputGPU, dataType *d_Input, dataType *d_Buffer, dataType *d_OutputGPU){
  cudaError_t err;
