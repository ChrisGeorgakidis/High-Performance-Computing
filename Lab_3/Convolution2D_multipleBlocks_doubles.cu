...
int main(int argc, char **argv) {
  ...
  *d_OutputGPU = NULL;
  ...
  //Allocate host (CPU) memory
  {
    ...
    h_Input     = (double *)malloc(ArraySize * sizeof(double));
    if (h_Input == NULL){
      printf("Error allocating memory on host for h_Input");
      freeMemory(h_Filter, h_Input, h_Buffer, h_OutputCPU, h_OutputGPU, d_Filter, d_Input, d_Buffer, d_OutputGPU);
      return (ERROR);
    }
    h_Buffer    = (double *)malloc(ArraySize * sizeof(double));
    if (h_Buffer == NULL){
      printf("Error allocating memory on host for h_Buffer");
      freeMemory(h_Filter, h_Input, h_Buffer, h_OutputCPU, h_OutputGPU, d_Filter, d_Input, d_Buffer, d_OutputGPU);
      return (ERROR);
    }
    h_OutputCPU = (double *)malloc(ArraySize * sizeof(double));
    if (h_OutputCPU == NULL){
      printf("Error allocating memory on host for h_OutputCPU");
      freeMemory(h_Filter, h_Input, h_Buffer, h_OutputCPU, h_OutputGPU, d_Filter, d_Input, d_Buffer, d_OutputGPU);
      return (ERROR);
    }
    h_OutputGPU = (double *)malloc(ArraySize * sizeof(double));
    if (h_OutputGPU == NULL){
  ...
  //Allocate device (GPU) memory
  {
    ...
    err = cudaMalloc( (void**) &d_Input, ArraySize * sizeof(double) );
    if (err != cudaSuccess){
      printf("Error allocating memory on host for d_Input:   %s\n", cudaGetErrorString(err));
      freeMemory(h_Filter, h_Input, h_Buffer, h_OutputCPU, h_OutputGPU, d_Filter, d_Input, d_Buffer, d_OutputGPU);
      return (ERROR);
    }
    err = cudaMalloc( (void**) &d_Buffer, ArraySize * sizeof(double) );
    if (err != cudaSuccess){
      printf("Error allocating memory on host for d_Buffer:   %s\n", cudaGetErrorString(err));
      freeMemory(h_Filter, h_Input, h_Buffer, h_OutputCPU, h_OutputGPU, d_Filter, d_Input, d_Buffer, d_OutputGPU);
      return (ERROR);
    }
    err = cudaMalloc( (void**) &d_OutputGPU, ArraySize * sizeof(double) );
    if (err != cudaSuccess){
  ...
  //Copy from host to device
  {
    ...
    err = cudaMemcpy(d_Input, h_Input, ArraySize * sizeof(double), cudaMemcpyHostToDevice);
    if (err != cudaSuccess)
    {
  ...
  //GPU Computation
  {
    ...
    err = cudaMemcpy(h_OutputGPU, d_OutputGPU, ArraySize * sizeof(double), cudaMemcpyDeviceToHost);
    if(err != cudaSuccess){
