...
{                                                                                             \
	...
	{                                                                                         \
		...
		printf("\033[0m");                                                                  \
		freeMemory(d_lut, d_outImage, d_inImage);                                       \
	}                                                                                         \
}
...
void freeMemory(int *d_lut, unsigned char *outputImage, unsigned char *inputImage)
{
	cudaError_t err;
...
PGM_IMG contrast_enhancement_g(PGM_IMG img_in)
{
	...
	//MemCpy lut Device to Device, since the result of prefix sum will be stored in __constant__ memory. No need for to memcpy, 1 cudaMemCpy DeviceToHost & 1 cudaMemCpyToSymbol HostToDevice
		//
		clock_gettime(CLOCK_MONOTONIC_RAW, &prefix_sum_DtoH_start);
		error = cudaMemcpyToSymbol(d_conLut, d_lut, HISTOGRAM_SIZE*sizeof(int), 0, cudaMemcpyDeviceToDevice);
		if (error != cudaSuccess)
		{
			printf("\033[1;31m");
			printf("Error during cudaMemcpy of d_lut to d_conLut:  %s\n", cudaGetErrorString(error));
			printf("\033[0m");
			freeMemory(d_lut, d_outImage, d_inImage);
			return (result);
		}
	...
	//MemCpy Host to Device for resultCalc
		...
		error = cudaMemcpy(d_inImage, img_in.img, imageSize*sizeof(unsigned char), cudaMemcpyHostToDevice);
		if (error != cudaSuccess)
		{
			printf("\033[1;31m");
			printf("Error during cudaMemcpy of img_in.img to d_inImage:  %s\n", cudaGetErrorString(error));
			printf("\033[0m");
			freeMemory(d_lut, d_outImage, d_inImage);
			return (result);
	...
	//Result Calculation in GPU
		resultCalc <<< hist_grid, hist_block >>> (d_outImage, d_inImage, imageSize);
	//__|
	...
	//MemCpy Device to Host
		error = cudaMemcpy(result.img, d_outImage, imageSize * sizeof(unsigned char), cudaMemcpyDeviceToHost);
		if (error != cudaSuccess)
		{
			printf("\033[1;31m");
