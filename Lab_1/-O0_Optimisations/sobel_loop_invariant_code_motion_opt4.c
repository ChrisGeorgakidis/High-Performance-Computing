...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	int temp3, temp4 = SIZE+1, temp5 = SIZE-1;
	int temp6 = temp5*SIZE;
	...
	for (i=1; i<temp5; i+=1) {
		...
		for (j=1; j<temp5; j+=1 ) {
			...
			temp1 = 0;
			temp1 += input[temp3 + j - temp4] * horiz_operator[0][0];
			temp1 += input[temp3 + j - SIZE] * horiz_operator[0][1];
			temp1 += input[temp3 + j - temp5] * horiz_operator[0][2];
			temp1 += input[temp3 + j - 1] * horiz_operator[1][0];
			temp1 += input[temp3 + j] * horiz_operator[1][1];
			temp1 += input[temp3 + j + 1] * horiz_operator[1][2];
			temp1 += input[temp3 + j + temp5] * horiz_operator[2][0];
			temp1 += input[temp3 + j + SIZE] * horiz_operator[2][1];
			temp1 += input[temp3 + j + temp4] * horiz_operator[2][2];


			temp2 = 0;
			temp2 += input[temp3 + j - temp4] * vert_operator[0][0];
			temp2 += input[temp3 + j - SIZE] * vert_operator[0][1];
			temp2 += input[temp3 + j - temp5] * vert_operator[0][2];
			temp2 += input[temp3 + j - 1] * vert_operator[1][0];
			temp2 += input[temp3 + j] * vert_operator[1][1];
			temp2 += input[temp3 + j + 1] * vert_operator[1][2];
			temp2 += input[temp3 + j + temp5] * vert_operator[2][0];
			temp2 += input[temp3 + j + SIZE] * vert_operator[2][1];
			temp2 += input[temp3 + j + temp4] * vert_operator[2][2];
			...
			if (res > 255)
				...
				output[temp3 + j] = (unsigned char)res;
		}
	}
	...
	for (i=1; i<temp5; i++) {
		temp3 = i*SIZE;
		for ( j=1; j<temp5; j++ ) {
			t = pow((output[temp3+j] - golden[temp3+j]),2);
			PSNR += t;
		}
	}
