...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	for (j=1; j<SIZE-1; j++) {
		for (i=1; i<SIZE-1; i++ ) {
			...
			temp1 = 0;
			temp1 += input[(i - 1)*SIZE + j - 1] * horiz_operator[0][0];
			temp1 += input[(i - 1)*SIZE + j] * horiz_operator[0][1];
			temp1 += input[(i - 1)*SIZE + j + 1] * horiz_operator[0][2];
			...
			temp1 += input[(i + 1)*SIZE + j - 1] * horiz_operator[2][0];
			temp1 += input[(i + 1)*SIZE + j] * horiz_operator[2][1];
			temp1 += input[(i + 1)*SIZE + j + 1] * horiz_operator[2][2];


			temp2 = 0;
			temp2 += input[(i - 1)*SIZE + j - 1] * vert_operator[0][0];
			temp2 += input[(i - 1)*SIZE + j] * vert_operator[0][1];
			temp2 += input[(i - 1)*SIZE + j + 1] * vert_operator[0][2];
			temp2 += input[(i)*SIZE + j - 1] * vert_operator[1][0];
			temp2 += input[(i)*SIZE + j] * vert_operator[1][1];
			temp2 += input[(i)*SIZE + j + 1] * vert_operator[1][2];
			temp2 += input[(i + 1)*SIZE + j - 1] * vert_operator[2][0];
			temp2 += input[(i + 1)*SIZE + j] * vert_operator[2][1];
			temp2 += input[(i + 1)*SIZE + j + 1] * vert_operator[2][2]; 
