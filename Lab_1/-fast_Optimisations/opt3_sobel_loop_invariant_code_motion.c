...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	for (j=1; j<temp5; j++) {
		...
		for (i=1; i<temp5; i++ ) {
			...
			temp1 = 0;
			temp1 += input[i*SIZE + j - temp4] * horiz_operator[0][0];
			temp1 += input[i*SIZE + j - SIZE] * horiz_operator[0][1];
			temp1 += input[i*SIZE + j - temp5] * horiz_operator[0][2];
			temp1 += input[i*SIZE + j - 1] * horiz_operator[1][0];
			temp1 += input[i*SIZE + j] * horiz_operator[1][1];
			temp1 += input[i*SIZE + j + 1] * horiz_operator[1][2];
			temp1 += input[i*SIZE + j + temp5] * horiz_operator[2][0];
			temp1 += input[i*SIZE + j + SIZE] * horiz_operator[2][1];
			temp1 += input[i*SIZE + j + temp4] * horiz_operator[2][2];


			temp2 = 0;
			temp2 += input[i*SIZE + j - temp4] * vert_operator[0][0];
			temp2 += input[i*SIZE + j - SIZE] * vert_operator[0][1];
			temp2 += input[i*SIZE + j - temp5] * vert_operator[0][2];
			temp2 += input[i*SIZE + j - 1] * vert_operator[1][0];
			temp2 += input[i*SIZE + j] * vert_operator[1][1];
			temp2 += input[i*SIZE + j + 1] * vert_operator[1][2];
			temp2 += input[i*SIZE + j + temp5] * vert_operator[2][0];
			temp2 += input[i*SIZE + j + SIZE] * vert_operator[2][1];
			temp2 += input[i*SIZE + j + temp4] * vert_operator[2][2];

			p = pow(temp1, 2) + pow(temp2, 2);
			res = (int)sqrt(p);
